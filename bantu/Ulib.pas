...
function cekbarcode(akode:String):Boolean;
...
begin
  Result := 0;
  s := 'select brg_hrgjualpcs from tbarang where brg_kode = ' + Quot(akode) ;
...
begin
  result := false;
  s:='select *  from tbarang where brg_barcode='+ Quot(akode) ;
  tsql := xOpenQuery(s,frmMenu.conn);
  with tsql do
  begin
   try
