...
uses
  ...
  dxSkinscxPCPainter, MyAccess;

type
  TfrmLapEstimasiMarketing = class(TForm)
    AdvPanel1: TAdvPanel;
    Label2: TLabel;
    Label3: TLabel;
    edtNama: TAdvEdit;
...
var
  frmLapEstimasiMarketing: TfrmLapEstimasiMarketing;
...
procedure TfrmLapEstimasiMarketing.FormCreate(Sender: TObject);
begin
     TcxDBGridHelper(cxGrdMain).LoadFromCDS(CDS, False, False);
end;

procedure TfrmLapEstimasiMarketing.refreshdata;
begin
  FID:='';
  edtKode.Clear;
  edtNama.Clear;
...
procedure TfrmLapEstimasiMarketing.initgrid;
begin
  CDS.EmptyDataSet;
  CDS.Append;
  CDS.Post;
...
procedure TfrmLapEstimasiMarketing.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmLapEstimasiMarketing.FormKeyPress(Sender: TObject; var Key: Char);
begin
   if Key = #13 then
      SelectNext(ActiveControl,True,True);
end;

procedure TfrmLapEstimasiMarketing.loaddata(akode:string) ;
var
  ssql,s: string;
  tsql2,tsql : TmyQuery;
  i:Integer;
...
//    if cekada(edtTahun.Text,IntToStr(cbbBulan.ItemIndex+1),akode) then
      s:= 'SELECT kode_grouppf, nama_grouppf, esd_targetsales target, esd_estimasisales estimasi, esd_realisasisales riil, '
        + ' CAST(0 AS DECIMAL) ratio,'
        + ' ('
        + ' SELECT ifnull(SUM(fpd_qty * hna_grouppf)/3,0)'
        + ' FROM ' + adatabase + '.tfp_dtl'
        + ' INNER JOIN ' + adatabase + '.tfp_hdr ON fp_nomor = fpd_fp_nomor'
        + ' INNER JOIN tbarangpf ON bpf_brg_kode = fpd_brg_kode AND bpf_tahun = '  + edtTahun.Text
        + ' AND bpf_periode = ' + IntToStr(cbbBulan.ItemIndex+1)
        + ' INNER JOIN tgrouppf ON kode_grouppf = bpf_kode_grouppf AND periode = ' + IntToStr(cbbBulan.ItemIndex+1)
        + ' AND tahun = '  + edtTahun.Text
        + ' inner join tsalescustomer on sc_cus_kode=fp_cus_kode and sc_periode='+ IntToStr(cbbBulan.ItemIndex+1) + ' and sc_tahun='+   edtTahun.Text
        + ' WHERE (fp_tanggal) < ' + Quot(edttahun.text + '/' + IntToStr(cbbBulan.ItemIndex+1) +  '/01')
        + ' AND fp_tanggal >= ' + Quot(IntToStr(atahun) + '/' + IntToStr(abulan) + '/01')
        + ' AND kode_grouppf = x.kode_grouppf and sc_sls_kode='+Quot(edtkode.text)+') avgsales'
        + ' FROM testimasimarketing_hdr2'
        + ' INNER JOIN testimasimarketing_dtl2 ON esd_esh_id = esh_id'
        + ' INNER JOIN tgrouppf  x ON kode_grouppf = esd_kode_grouppf  AND periode='+ IntToStr(cbbBulan.ItemIndex+1)+' AND tahun='+edttahun.Text
...
//    else
  ...
  begin
   ...
   S := 'SELECT tm_salesman,kode_grouppf,nama_grouppf,tm_target*hna_grouppf target,tm_periode,tm_tahun,'
        + ' ('
        + ' SELECT ifnull(SUM(fpd_qty * hna_grouppf)/3,0)'
        + ' FROM ' + adatabase + '.tfp_dtl'
        + ' INNER JOIN ' + adatabase + '.tfp_hdr ON fp_nomor = fpd_fp_nomor'
        ...
        + ' 0 estimasi,'
        + ' ('
        + ' SELECT ifnull(SUM(fpd_qty*hna_grouppf),0)'
        + ' FROM bsmcabang.tfp_dtl'
        + ' INNER JOIN bsmcabang.tfp_hdr ON fp_nomor= fpd_fp_nomor'
        + ' INNER JOIN tbarangpf ON bpf_brg_kode = fpd_brg_kode AND bpf_tahun = '  + edtTahun.Text
        + ' AND bpf_periode = ' + IntToStr(cbbBulan.ItemIndex+1)
        + ' INNER JOIN tgrouppf ON kode_grouppf = bpf_kode_grouppf AND periode = ' + IntToStr(cbbBulan.ItemIndex+1)
        + ' AND tahun = '  + edtTahun.Text
        + ' inner join tsalescustomer on sc_cus_kode=fp_cus_kode and sc_periode='+ IntToStr(cbbBulan.ItemIndex+1) + ' and sc_tahun='+   edtTahun.Text
        + ' WHERE MONTH(fp_tanggal) IN (' + IntToStr(cbbBulan.ItemIndex+1) + ') AND YEAR(fp_tanggal) = '  + edtTahun.Text
        + ' AND kode_grouppf = x.kode_grouppf  and sc_sls_kode='+Quot(edtkode.text)+') riil'
        + ' ,0 ratio'
        + ' FROM ttargetmarketing'
        + ' INNER JOIN tmarketing ON mkt_kode=tm_salesman'
        + ' INNER JOIN tgrouppf x ON kode_grouppf=tm_grouppf  AND tm_periode=periode AND tm_tahun=tahun'
        + ' WHERE mkt_kode = ' + Quot(edtkode.text)
...
//      CDS.EmptyDataSet;
        while  not Eof do
        begin
           if CDS.Locate('kode',FieldByName('kode_grouppf').AsString,[loCaseInsensitive]) then
             begin
...
procedure TfrmLapEstimasiMarketing.simpandata;
var
  s:string;
  i:integer;
  tt:TStrings;
...
procedure TfrmLapEstimasiMarketing.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action := caFree;
   Release;
end;

procedure TfrmLapEstimasiMarketing.cxButton1Click(Sender: TObject);
begin
    try
...
procedure TfrmLapEstimasiMarketing.cxButton8Click(Sender: TObject);
begin
Release;
end;

procedure TfrmLapEstimasiMarketing.cxButton2Click(Sender: TObject);
begin
   try
      If not cekdata(edtTahun.Text,IntToStr(cbbBulan.ItemIndex+1),edtKode.Text) then exit;
      if (FLAGEDIT) and ( not cekedit(frmMenu.KDUSER,self.name)) then
...
procedure TfrmLapEstimasiMarketing.edtKodeClickBtn(Sender: TObject);
begin
...
procedure TfrmLapEstimasiMarketing.FormShow(Sender: TObject);
begin
refreshdata;
end;

function TfrmLapEstimasiMarketing.GetCDS: TClientDataSet;
begin
...
procedure TfrmLapEstimasiMarketing.clCustomerPropertiesButtonClick(
...
procedure TfrmLapEstimasiMarketing.clNoGetDisplayText(Sender: TcxCustomGridTableItem;
...
procedure TfrmLapEstimasiMarketing.cxGrdMainKeyDown(Sender: TObject; var Key: Word;
    Shift: TShiftState);
begin
...
function TfrmLapEstimasiMarketing.gettarget(akode:string): double;
var
  s: string ;
  tsql : TmyQuery;
begin
...
function TfrmLapEstimasiMarketing.cekdata(atahun:string;abulan:string;akode:string):Boolean;
var
  i:integer;
  s:string;
  tsql:TmyQuery;
...
function TfrmLapEstimasiMarketing.cekada(atahun:string;abulan:string;akode:string):Boolean;
var
  i:integer;
  s:string;
  tsql:TmyQuery;
...
procedure TfrmLapEstimasiMarketing.cxGrdMainTcxGridDBDataControllerTcxDataSummaryFooterSummaryItems6GetText(
...
procedure TfrmLapEstimasiMarketing.cxGrdMainTcxGridDBDataControllerTcxDataSummaryFooterSummaryItems7GetText(
  Sender: TcxDataSummaryItem; const AValue: Variant; AIsFooter: Boolean;
  var AText: String);
  var
    capaibulanini :double;
...
procedure TfrmLapEstimasiMarketing.btnRefreshClick(Sender: TObject);
begin
  if edtkode.text <> '' then
