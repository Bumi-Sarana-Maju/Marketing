...
uses
  ...
  cxCurrencyEdit, cxGridDBBandedTableView, dxPScxGrid6Lnk, dxSkinBlack,
  ...
  dxSkinSummer2008, dxSkinValentine, dxSkinXmas2008Blue, cxTextEdit, MyAccess,
  MemDS, DBAccess;

type
  TfrmLapKunjungan = class(TForm)
...
var
  ...
  frmLapKunjungan: TfrmLapKunjungan;
...
procedure TfrmLapKunjungan.FormDblClick(Sender: TObject);
begin
  WindowState := wsMaximized;
end;

procedure TfrmLapKunjungan.btnExitClick(Sender: TObject);
begin
      Release;
end;

procedure TfrmLapKunjungan.refreshdata;
begin
  startdate.Date := date;
  enddate.Date := date;
...
procedure TfrmLapKunjungan.sbNewClick(Sender: TObject);
begin
   refreshdata;
...
procedure TfrmLapKunjungan.FormShow(Sender: TObject);
begin
  flagedit := False;
  refreshdata;
end;
...
procedure TfrmLapKunjungan.FormKeyPress(Sender: TObject; var Key: Char);
begin
    if Key = #13 then
...
procedure TfrmLapKunjungan.sbPrintClick(Sender: TObject);
begin
  refreshdata;
end;

procedure TfrmLapKunjungan.loaddata;
var
  ssql:string;
...
begin
  ...
  ssql := 'SELECT distinct USER Marketing,date_format(tanggal,"%Y-%m-%d") Tanggal,date_format(tanggal,"%H:%i:%s") Jam, '
+ ' cus_nama Customer,Catatan'
+ ' FROM zkunjungan a inner join customer  b'
+ ' ON a.cus_kode=b.cus_kode'
+ ' INNER JOIN tmarketing ON mkt_nama=user'
...
//    begin
//      ds3.Append;
//      ds3.FieldByName('marketing').AsString :=  fields[0].AsString;
//      ds3.FieldByName('nama').AsString :=  fields[1].AsString;
//
//      next;
//    end;
//  finally
...
procedure TfrmLapKunjungan.btnTampilClick(Sender: TObject);
begin
    loaddata;

end;

procedure TfrmLapKunjungan.cxGrid1DBBandedTableView1Column10CustomDrawCell(
...
procedure TfrmLapKunjungan.cxGrid1DBBandedTableView1Column10CustomDrawFooterCell(
...
procedure TfrmLapKunjungan.cxGrid1DBBandedTableView1Column13CustomDrawCell(
    Sender: TcxCustomGridTableView; ACanvas: TcxCanvas; AViewInfo:
    TcxGridTableDataCellViewInfo; var ADone: Boolean);
begin
...
procedure TfrmLapKunjungan.cxGrid1DBBandedTableView1Column13CustomDrawFooterCell(
    Sender: TcxGridTableView; ACanvas: TcxCanvas; AViewInfo:
    TcxGridColumnHeaderViewInfo; var ADone: Boolean);
begin
if StrToFloatDef(AViewInfo.Text,0) < 20 then
...
procedure TfrmLapKunjungan.cxGrid1DBBandedTableView1Column8CustomDrawCell(Sender:
    TcxCustomGridTableView; ACanvas: TcxCanvas; AViewInfo:
    TcxGridTableDataCellViewInfo; var ADone: Boolean);
begin
  if StrToFloatDef(AViewInfo.Text,0) < 70 then
...
procedure TfrmLapKunjungan.cxGrid1DBBandedTableView1Column8CustomDrawFooterCell(
    Sender: TcxGridTableView; ACanvas: TcxCanvas; AViewInfo:
    TcxGridColumnHeaderViewInfo; var ADone: Boolean);
begin
  if StrToFloatDef(AViewInfo.Text,0) < 70 then
...
procedure
    TfrmLapKunjungan.cxGrid1DBBandedTableView1TcxGridDBDataControllerTcxDataSummaryFooterSummaryItems11GetText(
    Sender: TcxDataSummaryItem; const AValue: Variant; AIsFooter: Boolean; var
    AText: string);
var
    capaibulanini :double;
...
procedure TfrmLapKunjungan.TeSpeedButton1Click(Sender: TObject);
begin
...
function TfrmLapKunjungan.GetPivotChartLink: TcxPivotGridChartConnection;
begin
...
procedure TfrmLapKunjungan.cxGrid1DBBandedTableView1TcxGridDBDataControllerTcxDataSummaryFooterSummaryItems8GetText(
...
procedure TfrmLapKunjungan.cxGrid1DBBandedTableView1TcxGridDBDataControllerTcxDataSummaryFooterSummaryItems9GetText(
  Sender: TcxDataSummaryItem; const AValue: Variant; AIsFooter: Boolean;
  var AText: String);
  var
    capaibulanini :double;
